...
public class HashTableSC<K, V> implements Map<K, V> {
	...
	public void put(K key, V value) {
		...
		if (key == null || value == null)
			throw new IllegalArgumentException("Parameter cannot be null.");
		double test = currentSize + 0.0;
		if(test / initialCapacity >= loadFactor) {
			rehash();
		}
	...
	private void rehash() {
		...
		buckets = new LinkedList[(int) (initialCapacity * 2)];
		this.initialCapacity = this.initialCapacity * 2;
		for (int i = 0; i < buckets.length; i++) {
			buckets[i] = new LinkedList<BucketNode<K, V>>();
		}
		for (List<BucketNode<K, V>> list : temp) {
			for (BucketNode<K, V> bucketNode : list) {
				int targetBucket = hashFunction.hashCode(bucketNode.getKey()) % buckets.length;
				List<BucketNode<K, V>> L = buckets[targetBucket];
				L.add(0, new BucketNode<K, V>(bucketNode.key, bucketNode.value));
			}
		}
	}
